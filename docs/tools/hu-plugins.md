...
See `crates/hiroz-union/plugins/hu-plugin-template/` for the minimal runnable version of the quick-start example above. CI builds it against the `wasm32-wasip2` target and then loads the compiled artifact through the real component-model host — `scripts/ci/hu-tests.sh` builds the plugin artifacts and runs `cargo test`, and it is the `hu_meter` test suite (`test_hu_plugin_template_validate_and_discover` in `crates/hiroz-tests/tests/hu_meter.rs`) that exercises `hu plugin validate` and `hu plugin list` against it. So the template is verified to load against the live WIT world, not merely to compile, and a WIT/ABI regression fails CI. `crates/hiroz-union/plugins/hu-meter/` and `crates/hiroz-union/plugins/hu-monitor/` are full CLI plugins with subcommand dispatch and periodic output.
