...
!!! warning "`--json` is accepted everywhere but honoured selectively"
    `--json` is a global flag, so the argument parser accepts it on *every* command — including ones that ignore it. Commands known to ignore it and always print human-readable text include `hu monitor watch`, `hu monitor log` and `hu monitor log-level` (of the `hu monitor` subcommands, only `graph` honours it), plus `hu meter echo`, `hu meter delay` and `hu meter param set`. Treat this as a non-exhaustive list: check a command's output before depending on it in a pipeline, because redirecting one of these into a `.jsonl` file yields human text, not JSON. For a machine-readable stream of graph change events, use [`hu stream --json`](hu.md#stream-mode) instead of `hu monitor watch --json`.
