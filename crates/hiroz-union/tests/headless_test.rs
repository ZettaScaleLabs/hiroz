...
/// Spawn `hu` in headless mode with echo topics.
fn spawn_hu_headless(router_endpoint: &str, echo_topics: &[&str]) -> ProcessGuard {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_hu"))
        .args(&args)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .process_group(0)
        .spawn()
        .expect("Failed to spawn hu");

    // Drain stderr in a background thread to prevent pipe buffer from filling up.
...
fn test_dynamic_subscriber_multiple_topics() {
    ...
    for line in reader.lines().take(100).flatten() {
        ...
        if let Ok(json) = serde_json::from_str::<Value>(&line) {
            if json["event"] == "topic_subscribed"
                && let Some(topic) = json["topic"].as_str()
            {
