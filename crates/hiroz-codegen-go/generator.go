...
func generateConstants(g *CodeBuilder, msg MessageDefinition) {
	// Use DDS-qualified type name so Zenoh key expressions match rmw_zenoh_cpp and hiroz Rust.
...
func GenerateGoService(srv ServiceDefinition, prefix string) ([]byte, error) {
	...
	// Use DDS-qualified service type name so Zenoh key expressions match rmw_zenoh_cpp and hiroz Rust.
	...
	// Service marker type implementing the hiroz.Service interface
	...
	g.P("func (s *%s) GetRequest() hiroz.Message { return &%s{} }", srv.Name, reqName)
	g.P("func (s *%s) GetResponse() hiroz.Message { return &%s{} }", srv.Name, respName)
	g.P("")
...
func GenerateGoAction(action ActionDefinition, prefix string) ([]byte, error) {
	...
	// Action-level constants: TypeName uses the hiroz internal format (not DDS).
	...
	g.P("func (a *%s) GetGoal() hiroz.Message { return &%s{} }", action.Name, goalName)
	if action.Result != nil {
		g.P("func (a *%s) GetResult() hiroz.Message     { return &%s{} }", action.Name, resultName)
	} else {
		g.P("func (a *%s) GetResult() hiroz.Message     { return nil }", action.Name)
	}
	if action.Feedback != nil {
		g.P("func (a *%s) GetFeedback() hiroz.Message   { return &%s{} }", action.Name, feedbackName)
	} else {
		g.P("func (a *%s) GetFeedback() hiroz.Message   { return nil }", action.Name)
	}
	g.P("")
