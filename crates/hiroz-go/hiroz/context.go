...
/*
#cgo LDFLAGS: -L${SRCDIR}/../../../target/release -lhiroz -lm
...
// Context represents a hiroz context
type Context struct {
	handle    *C.hiroz_context_t
	closeOnce sync.Once
}
...
func (b *ContextBuilder) Build() (*Context, error) {
	...
	if !b.hasAdvancedConfig {
		...
		handle = C.hiroz_context_create(C.uint32_t(b.domainID))
	} else {
		// Full config path
		pinner := &runtime.Pinner{}
		defer pinner.Unpin()

		var cfg C.hiroz_context_config_t
		...
		handle = C.hiroz_context_create_with_config(&cfg)
	}
...
func (c *Context) Close() error {
	...
	c.closeOnce.Do(func() {
		...
		result := C.hiroz_context_destroy(c.handle)
		c.handle = nil
		if result != 0 {
