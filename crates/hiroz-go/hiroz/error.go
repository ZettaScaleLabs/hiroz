...
// HirozError represents a structured error from the hiroz FFI layer
type HirozError struct {
	code ErrorCode
	msg  string
}

// Error implements the error interface.
// Format: "hiroz error N: message"
func (e HirozError) Error() string {
	return fmt.Sprintf("hiroz error %d: %s", e.code, e.msg)
}

// Code returns the FFI error code
func (e HirozError) Code() ErrorCode {
	return e.code
}

// Message returns the error message without the code
func (e HirozError) Message() string {
	return e.msg
}

// newHirozError creates a new HirozError with the given code and message.
...
func newHirozError(code ErrorCode, msg string) HirozError {
	return HirozError{code: code, msg: msg}
}
...
// Use errors.Is(err, hiroz.ErrTimeout) for idiomatic timeout checks.
func (e HirozError) Timeout() bool {
	return e.code == ErrorCodeServiceTimeout
}

// IsTimeout reports whether the error is a service call timeout.
//
// Deprecated: use Timeout() or errors.Is(err, hiroz.ErrTimeout) instead.
func (e HirozError) IsTimeout() bool {
	return e.Timeout()
}

// Is reports whether target matches this error by comparing error codes.
// This enables errors.Is() support for HirozError.
// Uses direct type assertion (not errors.As) to avoid recursive chain walking.
func (e HirozError) Is(target error) bool {
	t, ok := target.(HirozError)
	if ok {
...
// Use errors.Is(err, hiroz.ErrTimeout) etc. to check for specific conditions.
var (
	ErrTimeout      = newHirozError(ErrorCodeServiceTimeout, "service call timed out")
	ErrGoalRejected = newHirozError(ErrorCodeActionGoalRejected, "goal rejected")
	ErrResultFailed = newHirozError(ErrorCodeActionResultFailed, "action result failed")
	ErrCancelFailed = newHirozError(ErrorCodeActionCancelFailed, "action cancel failed")
	// ErrBuildFailed is returned when a builder's Build() call fails (FFI returned null pointer).
	// Use errors.Is(err, hiroz.ErrBuildFailed) to detect construction failures.
	ErrBuildFailed = newHirozError(ErrorCodeBuildFailed, "failed to build entity")
	// ErrCloseFailed is returned when a Close() call fails.
	// Use errors.Is(err, hiroz.ErrCloseFailed) to detect close failures.
	ErrCloseFailed = newHirozError(ErrorCodeCloseFailed, "close failed")
)
