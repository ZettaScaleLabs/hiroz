...
func (b *PublisherBuilder) Build(msg Message) (*Publisher, error) {
	...
	var handle *C.hiroz_publisher_t
	if b.qos != nil {
		cQos := b.qos.toCQos()
		handle = C.hiroz_publisher_create_with_qos(b.node.handle, topicC, typeNameC, typeHashC, &cQos)
	} else {
		handle = C.hiroz_publisher_create(b.node.handle, topicC, typeNameC, typeHashC)
	}
...
func (p *Publisher) Publish(msg Message) error {
	...
	result := C.hiroz_publisher_publish(
		p.handle,
...
func (p *Publisher) Close() error {
	...
	p.closeOnce.Do(func() {
		...
		result := C.hiroz_publisher_destroy(p.handle)
		p.handle = nil
		if result != 0 {
